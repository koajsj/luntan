...
function profile_setting($fieldid, $space = [], $showstatus = false, $ignoreunchangable = false, $ignoreshowerror = false) {
	...
	$encrypt = discuz_security_profile_encrypt_type($fieldid, $field['encrypt']);
	if(!empty($encrypt)) {
		$space[$fieldid] = authcode_field($encrypt, $space[$fieldid], 'DECODE');
	}
...
function profile_show($fieldid, $space = [], $getalone = false) {
	...
	$encrypt = discuz_security_profile_encrypt_type($fieldid, $_G['cache']['profilesetting'][$fieldid]['encrypt']);
	if(!empty($encrypt)) {
		$space[$fieldid] = authcode_field($encrypt, $space[$fieldid], 'DECODE');
	}
