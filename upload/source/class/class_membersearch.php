...
class membersearch {
	...
	function makeset($field, $condition, $type = 'string') {
		...
		if($return['table'] == 'profile' || $field == 'secmobile') {
			...
			$encrypt = discuz_security_profile_encrypt_type($_field, $fieldsetting['encrypt']);
		}
		...
		if($field == 'member.email') {
			if($islikesearch) {
				$likes = [];
				foreach($values as $value) {
					if(strexists($value, '*')) {
						$likes[] = "$field LIKE '".str_replace('*', '%', $value)."'";
					} else {
						$likes[] = "($field = '$value' OR $field LIKE '".discuz_sensitive_lookup(stripslashes($value), 'member.email')."')";
					}
				}
