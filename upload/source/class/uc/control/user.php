...
class usercontrol extends base {
	...
	function onedit() {
		...
		if($status > 0) {
			...
			$emailhash = $email !== '' ? substr(hash('sha256', strtolower(trim((string)$email))), 0, 16) : '';
			$mobilehash = $secmobile !== '' ? substr(hash('sha256', trim((string)$secmobicc).'-'.trim((string)$secmobile)), 0, 16) : '';
			$_ENV['user']->user_log($tmp['uid'], 'edituser', 'uid='.$tmp['uid'].'&emailhash='.$emailhash.'&mobilehash='.$mobilehash);
		}
		return $status;
	}
