...
class pmmodel {
	...
	function packpmmessage($message) {
		return addslashes(uc_encrypt_string($message, 'pm.message'));
	}

	function unpackpmmessage($message) {
		return uc_decrypt_string($message, 'pm.message');
	}

	function packlastmessage($lastmessage) {
		foreach(['firstsummary', 'lastsummary'] as $field) {
			if(isset($lastmessage[$field])) {
				$lastmessage[$field] = uc_encrypt_string($lastmessage[$field], 'pm.summary');
			}
		}
		return addslashes(serialize($lastmessage));
	}

	function unpacklastmessage($message) {
		$lastmessage = @unserialize($message);
		if(!is_array($lastmessage)) {
			return [];
		}
		foreach(['firstsummary', 'lastsummary'] as $field) {
			if(isset($lastmessage[$field])) {
				$lastmessage[$field] = uc_decrypt_string($lastmessage[$field], 'pm.summary');
			}
		}
		return $lastmessage;
	}
	...
	function getpostlist($list) {
		...
		foreach($list as $key => $value) {
			...
			$value['message'] = $this->unpackpmmessage($value['message']);
	...
	function lastpm($uid) {
		...
		$lastmessage = $this->unpacklastmessage($lastpm['lastmessage']);
	...
	function sendpm($fromuid, $fromusername, $touids, $subject, $message, $type = 0) {
		...
		$plaintextmessage = stripslashes($message);
		if(!$subject) {
			$subject = $this->removecode(trim($message), 80);
		} else {
			$subject = dhtmlspecialchars($subject);
		}
		$lastsummary = $this->removecode(trim($plaintextmessage), 150);
		$storedmessage = $this->packpmmessage($plaintextmessage);
		...
		if(!$type) {
			...
			$lastmessage = $this->packlastmessage($lastmessage);
			foreach($relationship as $key => $value) {
				if(!isset($existplid[$value])) {
					...
					$this->db->query('INSERT INTO '.UC_DBTABLEPRE.$this->getposttablename($plid)."(pmid, plid, authorid, message, dateline, delstatus) VALUES('$pmid', '$plid', '$fromuid', '$storedmessage', '".$this->base->time."', 0)");
	...
	function replypm($plid, $fromuid, $fromusername, $message) {
		...
		if($threadpm['pmtype'] == 1) {
			...
			$lastmessage = $this->unpacklastmessage($threadpm['lastmessage']);
	...
	function getpmlist($uid, $filter, $start, $ppp = 10) {
		...
		if($members) {
			...
			foreach($members as $key => $data) {
				...
				$data['pmid'] = $data['plid'];
				$lastmessage = $this->unpacklastmessage($data['lastmessage']);
